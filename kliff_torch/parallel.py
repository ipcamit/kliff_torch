...
def parmap1(f, X, *args, tuple_X=False, nprocs=mp.cpu_count()):
    ...
    not be idle worker and it should be faster than :meth:`kliff_torch.parallel.parmap2`.
    ...
    :meth:`kliff_torch.parallel.parmap2` that is based on ``multiprocessing.Pipe``.
...
def parmap2(f, X, *args, tuple_X=False, nprocs=mp.cpu_count()):
    ...
    the load in each process.  See :meth:`kliff_torch.parallel.parmap1` for another
